...
pars = []; pars.t = mean(pars_range); pars.dt = .5*(pars_range(2)-pars_range(1));
pars.used = 0; pars.level = -1; pars.hierarchy_hash = pars.level; pars.hierarchy = {1};

%%% train impedance model
[parsImpedance, appsImpedance] = buildFreqParsSurrogate(freq_test, pars, tol_freq, tol, ...
                                                        @getSampleImpedance_1p1, ...
                                                        @evaluatePiecewiseLinearInterpolant);
n_impedance = size(parsImpedance.t, 1);
[~, ~, vanderInvImpedance] = evaluatePiecewiseLinearInterpolant(0, parsImpedance, 1);

% prepare data structures for force surrogate
getForceWrapperArgs = {@getSampleForce_1p1, freq_0};
...
[~, ~, vanderInvForce] = evaluatePiecewiseLinearInterpolant(0, parsForce, 1);

%%% export results
out_filename = "output_1p1.mat";
